...
### 2026-09-06 Phase 0 baseline checkpoint

- 장 상태는 `weekend`, live runtime은 2026-09-04 15:30 KST 정상 종료 후 정지했다. watchdog/dashboard는 휴장일 stale·정지, startup launcher는 정상이다.
- trading mode는 `paper`, active h15는 `baseline-h15-v1`, 현재 통과한 수익 후보는 `0개`다.
- 2026-09-04 raw market/orderbook은 `3,811/4,049` symbol-minute, feature closed coverage는 `97.44%`, decision ledger `3,800/3,800`행의 complete lineage는 100%다.
- WebSocket reconnect `28`, storm `0`, 예상 밖 공통 gap 없음이며 재구독과 첫 프레임 복구가 모두 확인돼 `ATTENTION/주의`다.
...
- 2026-09-05 order-fill sync는 3페이지/38행, submission 38/38 exact-linked, open 0/final 38/pending 0으로 완결됐다.
- 2026-09-06 계좌 소유자 승인 marker-only clean baseline과 직후 reconciliation은 current epoch compatible, mismatch/effective cash/total asset gap `0`이다. Phase 0은 휴장일을 제외하고 `no_history`, `0/10`에서 다음 정상 거래일부터 누적한다.
- top challenger와 buy-avoid/buy-rescue/hold-rescue 모두 절대 비용 후 수익성 기준을 통과하지 못했다.
- E7은 미래 거래일 5일, official episode 0으로 표본 축적 중이다. 공식 `portfolio-replay-v2-minute-mtm`과 manifest는 일치한다.
...
- `run_phase1b_readonly_observation.sh` 기본 실행으로 네트워크 0회 사전검사를 먼저 통과시킨다.
