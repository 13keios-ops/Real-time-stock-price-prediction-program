...
- E6를 `trade_cost_pct=0.29`, `2 * cost=0.58`, `cost_model_version=krx-common-stock-2026-v1`로 다시 생성했다.
- KIS live h15는 `rows=79,422`, 중위 절대변동 `0.376648%`, p75 `0.721772%`다. 중위값은 보수 기준 미달이라 경고지만 상위 구간까지 포함해 h15 전체를 구조적으로 불가능하다고 확정하지 않는다.
- KIS live baseline-buy h15도 `rows=33,007`, 중위 `0.365344%`로 경고다. 현재처럼 빈번한 진입은 비용에 취약하다.
- KIS live h60은 `rows=69,962`, 중위 `0.739523%`, baseline-buy h60은 `rows=29,159`, 중위 `0.718133%`로 보수 기준을 넘는다. h60 연구 우선순위는 높지만 주문 정책 전환은 아니다.
- 수수료 `0.015%`와 편도 슬리피지 3bp를 연구 가정으로 명시했고, 관련 리포트 생성기에 `cost_model_version`과 가정 메타데이터를 추가했다. 진단 파일이 없을 때 구형 `0.108%`로 조용히 되돌아가던 fallback도 현행 `0.29%`로 교정했다.
- 2026-07-20 E1/E5 라운드 전 실험 동결, active `baseline-h15-v1`, 주문 정책·threshold·gate 무변경을 유지한다. 운영자 결정 사항은 없다.
