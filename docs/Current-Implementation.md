...
Broker paper order-fill sync retries KIS `EGW00201` rate-limit responses. If the broker still rejects the lookup, sync writes a `rate_limited` report and keeps submitted broker symbols pending instead of clearing them or crashing the runtime.
