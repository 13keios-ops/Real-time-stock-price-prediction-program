...
- broker paper 제출 응답이 network/unknown 오류로 유실되면 새 local order는 `submission_unknown`으로 남기고 동일 종목 재제출을 재시작·정기 sync 뒤에도 보류한다. 확정 KIS 거절은 기존 `rejected`로 구분한다. broker sync의 `unknown_local_submission_count`는 ack 없는 local attempt를 노출하며, reconciliation은 미확정 주문이 있으면 수치상 잔고가 같아도 `needs_review`다. 이 상태를 broker 체결로 추정하거나 자동 원장 보정하지 않는다.
...
- 장외 Phase 1b readiness cycle.
  `run_phase1b_readiness_cycle.sh` 기본 실행은 외부 KIS 네트워크 없이 local premarket/WS/preflight/fixture/readiness를 순서대로 갱신한다. 최신 data-quality에 reconnect가 있으면 같은 거래일의 storm 0, 연결 수, 재구독, 첫 프레임, 예상 밖 공통 gap 없음, 복구 후 raw frame, 30분 freshness를 모두 검증한 `real_kis_ws_recovery`를 우선 사용한다. 연결기 자체는 `network_called=false`이며 실제 세션 관측 lineage를 별도 표시한다. 복구 미관측이면 기존 synthetic probe를 사용하고, 불완전하거나 stale한 실제 증거는 synthetic으로 덮지 않고 fail-closed한다. 실제 live 조회는 `--execute`에서만 요청하고, `pre-open`/`regular-session`은 step 시작 전에 차단한다. preflight·실행 미시작·실제 실행 readiness 파일을 분리한다.
...
- 외국인·기관 수급 shadow 평가는 `python scripts/summarize_investor_flow_shadow.py --horizon-min 15`로 실행한다. KRX 공식 export의 finalized EOD 두 그룹을 `runtime-data/investor-flow/observations/investor_flow_eod.jsonl`에 넣으면, 기록된 `available_at` 이후 첫 closed label만 read-only 조인한다. 결과는 `runtime-data/reports/research/latest-investor-flow-shadow-h15.{json,md}`에 남긴다. E7 evaluator/manifest, model, feature, signal, gate, allocator, 주문 정책에는 연결하지 않는다.
