...
- 신호 정보계수 진단은 `python scripts/summarize_signal_ic.py --horizon-min 15`로 실행한다. 이 명령은 방어 shadow와 같은 baseline 매수 허용 row 에서 LightGBM 확률과 미래 수익률의 일별 Spearman 순위상관을 계산해 `runtime-data/reports/research/latest-signal-ic-h15.{json,md}`에 남긴다. 2026-07-05 기준 `probability_down`은 `mean_daily_ic=0.004754`, `t_stat=0.367342`, `decision=signal_quality_insufficient`라 E2/E3 threshold/EV 필터 튜닝으로 바로 넘어가지 않는다.
- 비용/시간지평 구조 진단은 `python scripts/summarize_cost_horizon_diagnostics.py --horizons 15 30 60`으로 실행한다. 이 명령은 `feature_labels`를 read-only 로 조회해 horizon별 절대 미래수익률 분포와 비용 기준을 비교하고 `runtime-data/reports/research/latest-cost-horizon-diagnostics.{json,md}`에 남긴다. 2026-07-05 review_ver_25 반영 기준 현재 schema에는 source 컬럼이 없어 `KIS live 근사 = serving runtime 심볼 + 2026-06-11 이후`, `Cybos historical 근사 = 2026-06-11 이전`으로 분리한다. 전체 h15 표본은 Cybos historical이 `99.02%`를 차지해 중위 절대수익률 `0.189394%`가 `2 * trade_cost_pct=0.216%`보다 작지만, KIS live 근사 h15 표본은 `rows=61,527`, `share=0.98%`, `median_abs=0.361446%`로 비용 기준을 넘었다(`decision=kis_live_h15_median_move_covers_2x_cost`). 따라서 E6만으로 h15 비용 구조만으로 배제하지 않고, E1 신호 품질 부족과 분리해서 본다. h60 KIS live 근사 표본은 `median_abs=0.717274%`이나 h60 정책 전환은 별도 검증 전까지 하지 않는다.
...
- buy-rescue 후속은 완전 폐기가 아니라 우선순위 하향이다. 다음 후보는 새 주문 정책이 아니라 `no-trade decision ledger`로 baseline 비매수/차단 판단을 기록할 수 있는지 확인하고, KIS live 에서 상승 후보 표본이 충분히 쌓인 뒤 다시 검토한다. 그 전까지 KIS live shadow 확장은 buy-avoid 순차 관측만 유지한다.
