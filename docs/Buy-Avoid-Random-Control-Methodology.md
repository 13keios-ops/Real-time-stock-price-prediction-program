...
- 현재 2026 보통주 세금은 매도 시 총 `0.20%`를 적용한다.
- 편도 수수료 `0.015%`와 편도 슬리피지 3bp는 브로커 확정값이나 실제 체결 보정값이 아닌 연구 가정이다.
- 리포트는 `cost_model_version`을 기록하고, Phase 2 canary에서 실제 계좌 현금 변화와 fill slippage로 재검증한다.
