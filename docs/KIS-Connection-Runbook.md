...
- KIS JSON 제어 프레임 PINGPONG은 수신 즉시 같은 payload의 WebSocket pong으로 응답하고, 시장 데이터·stable-frame·max-frame 집계에는 포함하지 않는다.
- `H0STCNT0` 다건 frame은 advertised record count와 총 token 수가 정확히 나뉠 때 actual row width로 row 경계를 계산하고, 문서상 필드 prefix만 저장한다. provider trailing field가 다음 row symbol/time으로 밀려들면 안 된다.
