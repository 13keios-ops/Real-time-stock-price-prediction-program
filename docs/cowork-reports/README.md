...
- Codex 작업 리포트 `work_ver_*`에는 결과만 쓰지 않는다. 반드시 `Codex 의견`, `다음 방향`, `계속 진행 기준`, `보류 기준`, `다음 cowork 리뷰 권장 시점`을 포함한다.
- cowork 리뷰가 단순 승인에 가까워도 Codex는 비판적 검토를 한 뒤, 지금 더 진행할지/월요일 장중·장후 증거를 기다릴지 같은 판단을 work report 또는 기준 문서에 남긴다.
