...
- 비공개 원격, 자격증명, 외부 미러, 비공개 산출물 세부는 git 추적 저장소에 쓰지 않고 형제 폴더 `../secrets/README.local.md`와 그 하위 로컬 문서에만 둔다.
...
- 비공개 원격, 비공개 대용량 자산, 자격증명, 외부 미러, 비공개 배포 운영이 포함된 작업은 형제 폴더 `../secrets/README.local.md`가 있을 때 함께 다시 읽는다.
...
- 로그성 데이터는 `logbook`에만 쌓고, `README.md`는 현재 사실 기준만 남긴다.
...
- 최종 답변 전에는 반드시 `현재 작업 모드`, `답변 접두어`, `활성 체크리스트 갱신 여부`, `기준 문서 반영 여부`를 한 줄 체크포인트로 다시 확인한다.
...
- 처음부터 과도한 추상화, 미사용 옵션, 추측성 예비 처리를 넣지 않는다.
...
- 민감한 운영 정보는 git 추적 저장소 밖 `../secrets/`에서만 관리한다.
...
- 연기 테스트, 디버그, 실패 실행 산출물은 최종 기준 산출물이 확보되면 삭제 가능한 임시 자산으로 본다.
- git으로 추적할 가치가 있는 소형 자산과 메타데이터만 저장소 안에 둔다.

## 7. 반복 작업은 skill 또는 스크립트로 분리한다

- 이 저장소의 공용 skill 원본 자리는 `repo/.agents/skills/` 아래에 둔다.
- 현재는 기준 skill 본문보다 실행 스크립트가 먼저 존재한다.
...
- 코드만 끝내지 않고 관련 기준 문서를 같은 턴에 함께 갱신한다.
- 산출물 경로, 다음 연결점, 삭제된 임시 자산이 있으면 `docs/logbook.md`에 남긴다.
- 변경된 파일이 있으면 가능한 한 같은 턴 안에 commit과 설정된 원격 저장소 push까지 마친다.
...
- 정책이 바뀌면 RECOVERY.md, scripts/run_weekly_nas_backup.ps1, scripts/run_forced_nas_backup.ps1 내용을 함께 맞춘다.
- 백업 정책이 바뀌면 RECOVERY.md, README.md, AGENTS.md, 백업 스크립트를 같은 작업에서 함께 갱신한다.
<!-- NAS_BACKUP_END -->
