...
- timeout, compaction 또는 사용자의 `계속`으로 작업을 재개할 때는 전달된 요약을 작업 인덱스로 사용하고 `git status`, 현재 diff, 최근 commit, 현재 스프린트의 미완료 항목만 먼저 검증한다. 불일치가 없으면 완료된 이력과 이미 읽은 문서를 다시 전면 탐색하지 않는다.
- 반복 구현 중에는 focused test를 사용하고 전체 test suite는 논리적 변경 묶음 종료 전에 1회만 실행한다. 중단 전에 같은 코드 상태로 완료한 검증은 다시 실행하지 않는다.
- 출력이 큰 명령은 임시 로그로 보내고 exit code와 마지막 요약을 우선 확인한다. 출력이 잘렸다는 이유만으로 성공한 장시간 명령을 곧바로 재실행하지 않는다.
- commit/push 경계마다 `STATUS`와 `SPRINT_CURRENT`가 다음 작업의 handoff가 되게 유지하고, 이후 별개 구현 묶음은 대화 전체 대신 이 기준 문서에서 시작한다.
