...
class OrchestratorTests(unittest.TestCase):
    def test_run_synthetic_dev_cycle(self) -> None:
        ...
        with patch.dict(os.environ, env, clear=False):
            ...
            self.assertTrue(str(result.training["model_version"]).startswith("lightgbm-h15-"))
            self.assertFalse(bool(result.training["activation_applied"]))
            ...
            self.assertGreaterEqual(len(result.challengers["candidates"]), 4)
            self.assertIn(result.challengers["recommended_action"], {"promote", "keep_active", "review_required"})
            self.assertIn(result.challengers["walk_forward_gate_status"], {"pass", "needs_review", "missing"})
