...
이 문서는 Codex와 외부 cowork가 같은 저장소를 검토할 때의 협업 절차를 정의한다.
사용자가 cowork 리뷰를 명시적으로 요청했거나 같은 주제의 review가 진행 중일 때만 적용하며, 일반 작업에는 review/work 산출물을 만들지 않는다.
저장소 내부 작업 규칙은 `AGENTS.md`가 최우선이며, 현재 사실은 `docs/STATUS.md`와 실제 리포트를 따른다.
